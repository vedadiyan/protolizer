...
func UnmarshalToMap(typeName string, bytes []byte) (map[string]any, error) {
	...
	case reflect.Struct:
		{
			...
			v, err := UnmarshalToMap(field.TypeName, value)
			if err != nil {
				return nil, pos, err
			}
			return v, pos + c, nil
...
func MarshalFromMap(typeName string, v map[string]any) ([]byte, error) {
	typ := CaptureTypeByName(typeName)
	out := make([]byte, 0)
	for _, i := range typ.Fields {
		var opts []encodeOption
	...
	case reflect.Struct:
		{
			...
			data, err := MarshalFromMap(typeName, v.Interface().(map[string]any))
			if err != nil {
				return nil, err
			}
			out := encodeBytes(data)
